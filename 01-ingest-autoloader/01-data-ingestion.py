...
# MAGIC
# MAGIC We'll can use Databricks Autoloader (`cloudFile` format) to incrementally load new data and append them to our raw tables. Re-running this job will only consume new data, handling all schema inference, evolution and scalability for us. 
# MAGIC
# MAGIC For this example, we'll use dbt to ingest the files directly, no need to uncomment the following cells, unless to build raw tables.
# MAGIC
